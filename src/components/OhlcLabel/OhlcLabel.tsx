...
export default function OhlcLabel(props: {
  label: OhlcLabelType;
  selectedCoin: string;
}) {
    ...
    <div>
      <p style={{ color: "grey" }}>
        ...
        <span style={{ color: "white" }}>
          {props.selectedCoin.substring(1, 4)}/{props.selectedCoin.substring(4)}
        </span>
        &nbsp;O <span style={colorAtt}>{props.label.open}</span>&nbsp; H{" "}
        <span style={colorAtt}>{props.label.high}</span>&nbsp; L{" "}
        <span style={colorAtt}>{props.label.low}</span>&nbsp; C{" "}
        <span style={colorAtt}>
          {props.label.close} {updatedDiff}(
          {isNeg ? diffPercentage : `+${diffPercentage}`}%)
        </span>
      </p>
    </div>
  );
